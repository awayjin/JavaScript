...
include('connect.php');
	...
	$result = mysql_query("select bookName,detail from books");
...
function table(){
	$str_table = "";
	$str_table .= "<table border='1' cellpadding='0' cellspacing='0' width='600' align='center'>";
	$str_table .= "<caption><h1>十行</h1></caption>";
	for($out = 0; $out < 10; $out++){
		$str_table .= "<tr>";
		for($in =0; $in<10; $in++){
			$str_table .= "<td align='center'>".($out*10 + $in)."</td>";
		}
		$str_table .= "</tr>";
	}
	$str_table .= "</table>";
...
<!--<form id="signupForm" class="vvform" method="get" action="">
    <p>
...
<form id="signupForm" class="vvform" method="post" action="c.php">
  	<p>
...
<!--    <p>
    	<label for="password">密码</label>
       	<input type="password" name="password" class="psd {required:true, minlength:6, maxlength:10}" />
    </p>
    
    <p>
    	<label for="confirm_password">确认密码</label>
        <input type="password" name="confirm_password" class="{required:true, minlength:6, maxlength:10, equalTo: '.psd'}" />
    </p>
-->    
	<p>
...
//alert(cc);
var cc ="<div class='wrap'><div class='find'>包装试试123</div></div>";
$("#test").click(function(){
	var bcc = $('.find', cc);
	$(this).html(bcc);
	bcc.append("<h2>212That is ok!</h2>")
	alert(bcc.html());
	/*$.ajax({
		type: "post",
		url: "Test_validation.html",
		success: function(cc){
			var bcc = $('.find', cc);
			alert(bcc);
		},
...
var jMax = jQuery.extend({ //扩展jQuery对象本身
	max: function(a, b){ if(a>b){alert(a)}}
});
//jMax.max(5,4);
