  ...
  $.openLayer = function(p){
		...
		var fs = {
			...
			add_data : function(targetid){
				...
				if(param.maxItem > 1){ fs.change_status(targetid); } //同步状态,单选状态无必要
				fs.add_input_event(targetid); //加入input的绑定事件
				fs.add_span_event(targetid); //span元素的绑定事件
			},
			
			change_status: function(){
				//切换不同元素，形成不同下级列表时候，同步已选择区的元素和新形成区元素的选中状态
				var selArea = $("#selArea"),
				 	selinputs = selArea.find(":input"),
				    vals =[];
...
//						  fs.add_data(next);
			...
			check_level: function(obj){//检查选择是否同一级城市(省份)
				//alert($(obj).attr("value"));	 //maybe 1702		
				//$("#selArea").find(":input[value="+$(obj).attr("value")+"]").parent().remove();
				...
				if(oneLevel == false){ //不限制城市选择级别
					var thisValue = $( obj ).attr( "value" ), //当前点击input元素(value值)
					    selValue = [];//存放已选择的input元素value值
					//var nameLevel = [];//存放被选中元素的name级别
					//var removSpan = [];
					$("#selArea").find("input").each(function(i){
						selValue.push($(this).attr("value")); //已选择的input元素valu值
						//var nameString = $(this).parent("span").attr("name"); //被选中元素的name级别
						//nameLevel.push(nameString);
						//alert(nameLevel[i]);
						if( selValue.length == 0 ){
							return true;	
						} else {
							if( (selValue.length == $("#selArea").children().length-1) && (selValue[i].substring(2,5) == '00') && (thisValue.substring(2,5) != '00' ) ){//点击input市级，删除包含有此市级的省份
							 	var provInput = $("#selArea").find(":input[value="+(thisValue.substring(0,2)+"00")+"]");
								var d2Val = thisValue.substring(0,2)+"00";
								//var a =$("#d2").find("input[value="+(d2Val)+"]").parent().remove();
								//alert($("#d1").find("input").attr("value"))
								
								var d1Arr = []; //存放省份的value值
								$("#d1").find("input").each(function(i){
									var vals = d1Arr.push( $(this).attr("value") );
									if( jQuery.inArray(d2Val, d1Arr) !== '-1'){
										$(this).attr("checked", false);	
									}
								});
								
								$("#d2").find("input[value="+(thisValue.substring(0,2)+"00")+"]").attr("checked", false); 
								//selValue[i].attr( "checked", false); 
								provInput.parent().remove();
								
																
							} else if( (thisValue.substring(2,5) == '00') && (selValue[i].substring(2,5) != '00') && (selValue[i].substring(0,2) == thisValue.substring(0,2)) ){//点击input省份，删除所有该省份下的市级元素
								$(this).parent("span").remove();
								
							}
							
							/*if( (nameLevel[i] == '0') && ($("#selArea").children().length-1 == nameLevel.length) && ($(obj).parent().attr("name") != '0') ){ //被选中的span元素全是name=0的级别，也就是选中的全是省份
								//alert(nameLevel[i]); //只有省份才能取到name值
								if(selValue[i].substring(0,2) == thisValue.substring(0,2)){
							...
							if(selValue[i].substring(0,2) == thisValue.substring(0,2) && (selValue[i].substring(2,5)=='00')){
								//alert(selValue[i]);
								$(this).parent().remove();
							}else if(selValue[i].substring(0,2) == thisValue.substring(0,2) && nameLevel[i] != '0'){
								//nameLevel[i].children().parent().remove();
								var removeSting = $(this).parent("span"); 
								removSpan.push(removeSting);
								removSpan[i].remove();
							}*/
						}
